...
class InvoiceAdmin(admin.ModelAdmin):
    ...
    def company_name(self, obj):
        ...
        if obj.company:
            ...
            return mark_safe(f'<a href="{url}">{obj.company.name}</a>')
        return '-'
    ...
    def total_amt_formatted(self, obj):
        ...
        if obj.total_amt:
            amount = float(obj.total_amt)
            return mark_safe(f'<span style="font-weight: bold;">{currency} {amount:.2f}</span>')
        return '-'
    ...
    def balance_formatted(self, obj):
        ...
        if obj.balance is not None:
            ...
            return mark_safe(f'<span style="{style}">{currency} {balance:.2f}</span>')
        return '-'
    ...
    def connection_status(self, obj):
        ...
        if obj.company and obj.company.is_connected:
            return mark_safe('<span style="color: green;">● Connected</span>')
        return mark_safe('<span style="color: red;">● Disconnected</span>')
    ...
    def raw_data_formatted(self, obj):
        ...
        if obj.raw_data:
            ...
            # Escape HTML and create safe string
            from django.utils.html import escape
            escaped_formatted = escape(formatted)
            
            return mark_safe(f'<pre style="background: #f8f8f8; padding: 10px; overflow: auto; max-height: 400px;">{escaped_formatted}</pre>')
        return "No data"
...
class InvoiceLineAdmin(admin.ModelAdmin):
    ...
    def invoice_doc_number(self, obj):
        ...
        doc_number = obj.invoice.doc_number or f'Invoice #{obj.invoice.qb_invoice_id}'
        return mark_safe(f'<a href="{url}">{doc_number}</a>')
    ...
    def unit_price_formatted(self, obj):
        ...
        if obj.unit_price:
            ...
            return f"{currency} {unit_price:.2f}"
        return '-'
