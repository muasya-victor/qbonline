...
class QuickBooksCreditNoteService:
    ...
    def fetch_credit_notes_from_qb(self) -> List[Dict[str, Any]]:
        ...
        while True:
            ...
            try:
                ...
                logger.info(f"Retrieved {len(credits)} credit notes (total so far: {len(all_credits)})")
    ...
    def sync_credit_note_to_db(self, credit_data: Dict) -> CreditNote:
        """Sync a single credit note with tax information"""
        try:
            ...
            # Extract tax information for credit note
        ...
        try:
            ...
            for data in credits_data:
                try:
                    ...
                    failed += 1
                    logger.error(f"Failed to sync credit note {data.get('DocNumber', 'Unknown')}: {str(e)}")
                    continue
            
            logger.info(f"Credit note sync completed: {synced} successful, {failed} failed")
            return synced
